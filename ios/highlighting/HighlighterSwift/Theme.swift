    ...
    */
    private func stripTheme(_ themeString : String) -> HRThemeStringDict {
        ...
        for result in results where result.numberOfRanges == 3 {
            ...
            let declarationBlock = nsTheme.substring(with: result.range(at: 2))
            for pair in declarationBlock.split(separator: ";", omittingEmptySubsequences: true) {
            ...
            for rawSelector in selectorGroup.split(separator: ",", omittingEmptySubsequences: true) {
                let selector = rawSelector.trimmingCharacters(in: .whitespacesAndNewlines)
                if selector.isEmpty {
                    continue
                }

                let selectorRange = NSRange(location: 0, length: selector.utf16.count)
                let classMatches = classSelectorRegex.matches(in: selector, options: [], range: selectorRange)
                if classMatches.isEmpty {
                    continue
                }

                // Combinators target the right-most selector (e.g. ".hljs-meta .hljs-keyword").
                // For single compound selectors (e.g. ".hljs-title.class_"), prefer the base class.
                let hasCombinator =
                    selector.contains(" ") || selector.contains(">") || selector.contains("+") || selector.contains("~")
                let chosenMatch = hasCombinator ? classMatches.last! : classMatches.first!
                let selectorNSString = selector as NSString
                let key = selectorNSString.substring(with: chosenMatch.range)

                var existing = returnDict[key] ?? [:]
                for (propName, propValue) in attributes where existing[propName] == nil {
                    existing[propName] = propValue
                }
                returnDict[key] = existing
            }
        }
