            ...
            if nextChar == "s" {
                ...
                if let content = scanned, !content.isEmpty {
                    let classNames = content
                        ...
                        .map(String.init)
                        .filter { !$0.isEmpty }
                    propStack.append(contentsOf: classNames)
                    spanClassCounts.append(classNames.count)
                } else {
                    spanClassCounts.append(0)
                }
            } else if nextChar == "/" {
                // We have a SPAN end tag so skip over it
                _ = scanner.scanString(self.spanEnd)
                let classCount = spanClassCounts.popLast() ?? 1
                let removable = min(classCount, max(propStack.count - 1, 0))
                if removable > 0 {
                    propStack.removeLast(removable)
                }
            } else {
