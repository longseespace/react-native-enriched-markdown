...
id RichTextViewShadowNode::setupMockRichTextView_(CGFloat width) const
{
  RichTextView *mockView = [[RichTextView alloc] initWithFrame:CGRectMake(20000, 20000, width, 1000)];
  ...
  const auto &typedProps = *std::static_pointer_cast<const RichTextViewProps>(props);
                                            ...
                                            const LayoutConstraints &layoutConstraints) const
{
  ...
  void (^measureBlock)(void) = ^{
    if (view) {
      ...
      RichTextView *mockView = setupMockRichTextView_(maxWidth);
