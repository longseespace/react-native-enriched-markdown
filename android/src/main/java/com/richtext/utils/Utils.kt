...
/**
 * Adds spacing between list items by inserting spacer newlines and applying MarginBottomSpan.
 * Only applies spacing to items at the specified depth level.
 */
fun <T : Any> applyListItemSpacing(
  builder: SpannableStringBuilder,
  listStart: Int,
  listEnd: Int,
  ...
  if (itemMarginBottom <= 0) return

  val listSpans =
    builder
      .getSpans(listStart, listEnd, spanClass)
      .filter { span ->
        when (span) {
          is com.richtext.spans.UnorderedListSpan -> span.depth == depth
          is com.richtext.spans.OrderedListSpan -> span.depth == depth
          else -> false
        }
  ...
  // Apply spacing after each item except the last one
  listSpans.dropLast(1).reversed().forEach { span ->
    val itemEnd = builder.getSpanEnd(span)
    applySpacingAfterItem(builder, itemEnd, itemMarginBottom)
  }
}

private fun applySpacingAfterItem(
  builder: SpannableStringBuilder,
  itemEnd: Int,
  itemMarginBottom: Float,
) {
  val hasNewline = itemEnd > 0 && builder[itemEnd - 1] == '\n'

  if (hasNewline) {
    val existingSpans = builder.getSpans(itemEnd - 1, itemEnd, MarginBottomSpan::class.java)
    if (existingSpans.isEmpty()) {
      builder.setSpan(
        MarginBottomSpan(itemMarginBottom),
        itemEnd - 1,
        itemEnd,
        SPAN_FLAGS_EXCLUSIVE_EXCLUSIVE,
      )
